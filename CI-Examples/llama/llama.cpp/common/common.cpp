...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--erebor-ioctl") {
        params.erebor_ioctl = true;
        // get prompt input from the pseudo filesystem
        if ((dev_fd = open(IO_DEV, O_RDWR)) < 0) {
            fprintf(stderr, "error: failed to open Erebor I/O device file '%s'\n", IO_DEV);
            fprintf(stderr, "Please check if your kernel/libOS is configured correctly.\n");
            invalid_param = true;
            return true;
        }

        io_payload_t input_payload;
        input_payload.buf = (char *)malloc(0x200000 * sizeof(char));
        if (input_payload.buf) {
            memset(input_payload.buf, 0, 0x200000 * sizeof(char));
            input_payload.size = 0x200000;
        } else {
            fprintf(stderr, "error: failed to allocate memory for Erebor I/O device file '%s'\n", IO_DEV);
            invalid_param = true;
            return true;
        }

        // Retrieve the data using ioctl
        if (ioctl(dev_fd, 0xBAC0, &input_payload) < 0) {
            fprintf(stderr, "error: ioctl call failed for device '%s'\n", IO_DEV);
            invalid_param = true;
            free(input_payload.buf);
            return true;
        }

        // Construct the string from the buffer using its length
        // params.prompt = std::string(input_payload.buf, strlen(input_payload.buf));
        params.prompt = input_payload.buf;
        free(input_payload.buf);  // Free allocated memory
        
        // Trim a trailing newline, similar to the -f branch
...
void gpt_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    printf("  --erebor-ioctl\n");
    printf("                        prompt/data input from Erebor ioctl.\n");
    ...
    log_print_usage();
#endif // LOG_DISABLE_LOGS
    printf("Erebor sandbox options:\n");
    printf("  --erebor-ioctl        Prompt data input/output from Erebor ioctl\n");
}
